...
function sendFile(response, filePath) {
  if (!fs.existsSync(filePath)) {
    sendJson(response, 404, { error: "ҳ�治���ڡ�" });
    return;
  }
...
function assertOneOf(value, allowed, fieldName) {
  if (!allowed.includes(value)) {
    throw new Error(`${fieldName}����֧�ַ�Χ�ڡ�`);
  }
  return value;
}
...
function normalizeHotelDetails(hotelDetails, index, baseCurrency) {
  if (!Array.isArray(hotelDetails) || hotelDetails.length === 0) {
    throw new Error(`�� ${index + 1} ���Ƶ���Ŀ������¼��һ��������ϸ��`);
  }
  ...
  return hotelDetails.map((detail, detailIndex) => {
    ...
    const currency = assertSupportedCurrency(detail.currency || baseCurrency, `�� ${index + 1} ���Ƶ���Ŀ�� ${detailIndex + 1} ��������ϸ�ı���`);

    if (!detail.roomType || String(detail.roomType).trim() === "") {
      throw new Error(`����д�� ${index + 1} ���Ƶ���Ŀ�� ${detailIndex + 1} ��������ϸ�ķ������ơ�`);
    }
    if (roomCount <= 0 || nights <= 0) {
      throw new Error(`�� ${index + 1} ���Ƶ���Ŀ�� ${detailIndex + 1} ��������ϸ�ķ������������������ 0��`);
    }
    if (costNightlyRate < 0 || priceNightlyRate < 0) {
      throw new Error(`�� ${index + 1} ���Ƶ���Ŀ�� ${detailIndex + 1} ��������ϸ��ÿ���ɱ����ۺ����۵��۲���Ϊ������`);
    }
...
function normalizeVehicleDetails(vehicleDetails, index, baseCurrency) {
  if (!Array.isArray(vehicleDetails) || vehicleDetails.length === 0) {
    throw new Error(`�� ${index + 1} ���ó���Ŀ������¼��һ���ó���ϸ��`);
  }
  ...
  return vehicleDetails.map((detail, detailIndex) => {
    ...
    const detailType = assertOneOf(detail.detailType || "pickup", supportedVehicleDetailTypes, `�� ${index + 1} ���ó���Ŀ�� ${detailIndex + 1} ����ϸ���ó�����`);
    const pricingUnit = assertOneOf(detail.pricingUnit || "trip", supportedVehiclePricingUnits, `�� ${index + 1} ���ó���Ŀ�� ${detailIndex + 1} ����ϸ�ļƼ۵�λ`);
    const currency = assertSupportedCurrency(detail.currency || baseCurrency, `�� ${index + 1} ���ó���Ŀ�� ${detailIndex + 1} ����ϸ�ı���`);

    if (!detail.vehicleModel || String(detail.vehicleModel).trim() === "") {
      throw new Error(`����д�� ${index + 1} ���ó���Ŀ�� ${detailIndex + 1} ����ϸ�ĳ��͡�`);
    }
    if (vehicleCount <= 0) {
      throw new Error(`�� ${index + 1} ���ó���Ŀ�� ${detailIndex + 1} ����ϸ�ĳ������������ 0��`);
    }
    if (costUnitPrice < 0 || priceUnitPrice < 0) {
      throw new Error(`�� ${index + 1} ���ó���Ŀ�� ${detailIndex + 1} ����ϸ�ĳɱ����ۺ����۵��۲���Ϊ������`);
    }
...
function normalizeServiceDetails(serviceDetails, index, baseCurrency) {
  ...
  return serviceDetails.map((detail, detailIndex) => {
    ...
    const costUnitPrice = Number(detail.costUnitPrice || 0);
    const priceUnitPrice = Number(detail.priceUnitPrice || 0);
    const serviceRole = assertOneOf(detail.serviceRole || "guide", supportedServiceRoles, `�� ${index + 1} ������/������Ŀ�� ${detailIndex + 1} ����ϸ�ķ����ɫ`);
    const serviceLanguage = assertOneOf(detail.serviceLanguage || "zh", supportedServiceLanguages, `�� ${index + 1} ������/������Ŀ�� ${detailIndex + 1} ����ϸ������`);
    const serviceDuration = assertOneOf(detail.serviceDuration || "full_day", supportedServiceDurations, `�� ${index + 1} ������/������Ŀ�� ${detailIndex + 1} ����ϸ�ķ���ʱ��`);
    const currency = assertSupportedCurrency(detail.currency || baseCurrency, `�� ${index + 1} ������/������Ŀ�� ${detailIndex + 1} ����ϸ�ı���`);

    if (quantity <= 0) {
      throw new Error(`�� ${index + 1} ������/������Ŀ�� ${detailIndex + 1} ����ϸ������������� 0��`);
    }
    if (costUnitPrice < 0 || priceUnitPrice < 0) {
      throw new Error(`�� ${index + 1} ������/������Ŀ�� ${detailIndex + 1} ����ϸ�ĳɱ����ۺ����۵��۲���Ϊ������`);
    }
...
function normalizeQuoteItems(items, baseCurrency) {
  ...
  return items.map((item, index) => {
    const itemType = assertOneOf(item.type, supportedQuoteItemTypes, `�� ${index + 1} ��������Ŀ�ķ�������`);
    ...
    const currency = assertSupportedCurrency(item.currency || baseCurrency, `�� ${index + 1} ��������Ŀ�ı���`);

    if (!hasHotelDetails && !hasVehicleDetails && !hasServiceDetails) {
      if (quantity <= 0) {
        throw new Error(`�� ${index + 1} ��������Ŀ������������� 0��`);
      }
      if (cost < 0 || price < 0) {
        throw new Error(`�� ${index + 1} ��������Ŀ�Ľ���Ϊ������`);
      }
    }

    return {
      type: itemType,
      name: notEmpty(item.name, `�� ${index + 1} ��������Ŀ�ķ�������`),
      unit: hasHotelDetails ? "�Ƶ�" : hasVehicleDetails ? "�ó�" : notEmpty(item.unit || "��", `�� ${index + 1} ��������Ŀ�ĵ�λ`),
...
function normalizeQuotePayload(payload, existingId) {
  const currency = assertSupportedCurrency(payload.currency || "EUR", "���۱���");
  const language = assertOneOf(payload.language || "zh-CN", supportedLanguages, "�ĵ��������");
  const startDate = formatDate(notEmpty(payload.startDate || payload.tripDate, "�г̿�ʼ����"));
  const endDate = formatDate(notEmpty(payload.endDate || payload.tripDate, "�г̽�������"));

  if (endDate < startDate) {
    throw new Error("�г̽������ڲ��������г̿�ʼ���ڡ�");
  }
  ...
  return {
    ...
    clientName: notEmpty(payload.clientName, "�ͻ�����"),
    projectName: notEmpty(payload.projectName, "��Ŀ����"),
    contactName: notEmpty(payload.contactName, "��ϵ������"),
    ...
    destination: notEmpty(payload.destination || "Belgrade", "��ҪĿ�ĵ�"),
...
function normalizeTemplateItems(items) {
  if (!Array.isArray(items)) {
    throw new Error("ģ�屨����Ŀ��ʽ����ȷ��");
  }

  return items.map((item, index) => ({
    type: assertOneOf(item.type, supportedQuoteItemTypes, `�� ${index + 1} ��ģ����Ŀ�ķ�������`),
    name: notEmpty(item.name, `�� ${index + 1} ��ģ����Ŀ��Ĭ�Ϸ�������`),
    unit: notEmpty(item.unit || "��", `�� ${index + 1} ��ģ����Ŀ�ĵ�λ`),
    currency: assertSupportedCurrency(item.currency || "EUR", `�� ${index + 1} ��ģ����Ŀ�ı���`),
...
function normalizeReceptionPayload(payload, existingId) {
  return {
    ...
    projectId: payload.projectId ? String(payload.projectId).trim() : "",
    taskType: assertOneOf(payload.taskType || "airport_pickup", supportedReceptionTaskTypes, "��������"),
    title: notEmpty(payload.title, "�������"),
    assignee: notEmpty(payload.assignee, "������"),
    dueTime: notEmpty(payload.dueTime, "��ֹʱ��"),
    status: assertOneOf(payload.status || "pending", supportedReceptionStatuses, "����״̬"),
    location: notEmpty(payload.location, "�ص�"),
    notes: String(payload.notes || "").trim(),
  };
}
...
function normalizeDocumentPayload(payload, existingId) {
  return {
    ...
    title: notEmpty(payload.title, "�ĵ�����"),
    category: notEmpty(payload.category, "�ĵ�����"),
    language: assertOneOf(payload.language || "zh-CN", supportedLanguages, "�ĵ�����"),
    updatedAt: notEmpty(payload.updatedAt, "����ʱ��"),
  };
}
...
function deriveProjects(data) {
  ...
  return quotes.map((quote) => {
    ...
    return {
      ...
      specialRequirements: quote.notes || "��",
...
function getProjectDetail(data, projectId) {
  ...
  const project = findById(projects, projectId, "��Ŀ����");
...
async function handleApi(request, response, url) {
  ...
  if (request.method === "GET") {
    ...
    if (quoteId) {
      try {
        sendJson(response, 200, enrichQuote(findById(data.quotes, quoteId, "����")));
      } catch (error) {
        sendJson(response, 404, { error: error.message });
      }
      return true;
  ...
  if (request.method === "DELETE") {
    ...
    if (quoteId) {
      if (!removeById(data.quotes, quoteId)) {
        sendJson(response, 404, { error: "���۲����ڡ�" });
        return true;
      }
      saveSeedData(data);
  ...
  if (request.method === "GET") {
    ...
    if (receptionId) {
      try {
        sendJson(response, 200, findById(data.receptions, receptionId, "�Ӵ�����"));
      } catch (error) {
        sendJson(response, 404, { error: error.message });
      }
      return true;
  ...
  if (request.method === "DELETE") {
    ...
    if (receptionId) {
      if (!removeById(data.receptions, receptionId)) {
        sendJson(response, 404, { error: "�Ӵ����񲻴��ڡ�" });
        return true;
      }
      saveSeedData(data);
      sendJson(response, 200, { message: "�Ӵ�������ɾ����" });
      return true;
    }
  }
  ...
  if (request.method === "GET" && url.pathname === "/api/document-previews") {
    ...
    try {
      const quote = quoteId ? enrichQuote(findById(data.quotes, quoteId, "����")) : null;
      const reception = receptionId ? findById(data.receptions, receptionId, "�Ӵ�����") : null;
  ...
  if (request.method === "PUT") {
    ...
    if (documentId) {
      ...
      if (!existing) {
        sendJson(response, 404, { error: "�ĵ������ڡ�" });
        return true;
      }
