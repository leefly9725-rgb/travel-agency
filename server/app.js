...
async function handleApi(request, response, url) {
  ...
  const validAdvertisingDate = (value) => {
  ...
  if (request.method === "POST" && url.pathname === "/api/advertising/quotes/calculate") {
    try { const received = parseJsonBody(await readRequestBody(request)); const body = received.pricingEngine === "bom_v2" ? { ...received, quoteDate: canonicalAdvertisingV2Date(received) } : received; const catalog = await loadAdvertisingCalculationCatalog(body); const calculated = await calculateAdvertisingByEngine(body, catalog); sendJson(response, 200, sanitizeAdvertising(calculated.calculationSnapshot)); }
  ...
  if (request.method === "POST" && url.pathname === "/api/advertising/quotes") {
    try { const received = parseJsonBody(await readRequestBody(request)); const body = received.pricingEngine === "bom_v2" ? { ...received, quoteDate: canonicalAdvertisingV2Date(received) } : received; assertAdvertisingRequiredFields(body); const catalog = await loadAdvertisingCalculationCatalog(body); const serverBody = buildAdvertisingServerSnapshot(body, catalog); const calculated = await calculateAdvertisingByEngine(serverBody, catalog); const saved = await advertisingStore.saveQuote({ ...calculated.safeBody, ...(body.pricingEngine === "bom_v2" ? { entitySnapshot: serverBody.entitySnapshot, termsSnapshot: serverBody.termsSnapshot } : {}), id: undefined, quoteNumber: undefined, ownerId: authCtx.userId, calculationSnapshot: calculated.calculationSnapshot, ...(body.pricingEngine === "bom_v2" ? { pricingEngine: "bom_v2", fxSnapshot: calculated.fxSnapshot, bomLines: calculated.bomLines } : {}), createdBy: authCtx.userId, updatedBy: authCtx.userId }); sendJson(response, 201, sanitizeAdvertising(saved)); }
    ...
    if (match) {
      ...
      try {
        ...
        if ((request.method === "PUT" && !action) || (request.method === "POST" && action === "calculate")) {
          const body = parseJsonBody(await readRequestBody(request)); if (!action) assertAdvertisingRequiredFields(body); const existing = assertAdvertisingOwnership(await advertisingStore.getQuote(id)); const calculationBody = { ...body }; if (existing.pricingEngine === "bom_v2") { calculationBody.pricingEngine = "bom_v2"; calculationBody.quoteDate = canonicalAdvertisingV2Date(body, existing); } else delete calculationBody.pricingEngine; const catalog = await loadAdvertisingCalculationCatalog(calculationBody); const serverBody = action ? calculationBody : buildAdvertisingServerSnapshot(calculationBody, catalog, existing); const calculated = await calculateAdvertisingByEngine(serverBody, catalog, existing);
