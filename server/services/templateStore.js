...
async function supabaseRequest(config, pathname, options = {}) {
  const key = getRemoteKey(config);
  if (!config.url || !key) {
    throw new Error("Supabase 未配置，无法使用云端模板存储。");
  }
  ...
  return {
    async listTemplates() {
      ...
      try {
        ...
        console.warn("模板读取出错，回退到本地 JSON。", error.message);
    ...
    async saveTemplate(template) {
      ...
      try {
        ...
        console.warn("模板保存出错，回退到本地 JSON。", error.message);
    ...
    async deleteTemplate(templateId) {
      ...
      try {
        ...
        console.warn("模板删除出错，回退到本地 JSON。", error.message);
