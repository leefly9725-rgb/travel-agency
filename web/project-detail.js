...
function renderStatusSelect(currentStatus, projectId) {
  const options = VALID_STATUSES.map((s) => `
    <option value="${s}" ${s === currentStatus ? "selected" : ""}>${PROJECT_STATUS_LABELS[s] || s}</option>
  `).join("");
  return `
    <div class="status-switch-row" style="display:flex;align-items:center;gap:8px;margin-top:8px;">
      <label for="status-select" style="font-size:13px;color:#666;">状态</label>
      <select id="status-select" data-project-id="${esc(projectId)}" style="font-size:13px;padding:4px 8px;border-radius:6px;border:1px solid #ccc;">
        ${options}
      </select>
      <span id="status-save-hint" style="font-size:12px;color:#999;"></span>
    </div>
  `;
}
  ...
  return projectGroups.map((group) => {
    ...
    const rows = items.map((item) => `
      <tr>
        ...
        <td>${esc(item.itemType || item.item_type || "")}</td>
...
function renderRealProject(project) {
  ...
  return `
    <section class="panel">
      ...
      ${renderStatusSelect(project.status, project.id)}
