...
function formatMoney(value, currency) {
  return window.AppUtils.formatCurrency(Number(value) || 0, currency);
}
...
function addAction(parent, element) {
  (parent || document.getElementById("quote-action-row"))?.appendChild(element);
}

function createActionElement(tag, text, className) {
  const el = document.createElement(tag);
  el.className = `qd-action workflow-btn ${className || "qd-action-secondary"}`;
  el.textContent = text;
  return el;
}

function renderDetailMeta(values) {
  return values.filter(value => value != null && value !== "").map(value => `<span>${esc(value)}</span>`).join("");
}

function renderCompactDetailRows(rows, quoteCurrency) {
  if (!Array.isArray(rows) || rows.length === 0) return "";
  return `
    <div class="quote-detail-subrows">
      ${rows.map(row => `
        <div class="quote-detail-subrow">
          <div>
            <strong>${esc(row.title)}</strong>
            <div class="quote-detail-submeta">${renderDetailMeta(row.meta)}</div>
            ${row.note ? `<p class="quote-detail-note">${esc(row.note)}</p>` : ""}
          </div>
          <div class="quote-detail-subamounts">
            <span>成本 ${formatMoney(row.cost, quoteCurrency)}</span>
            <strong>销售 ${formatMoney(row.price, quoteCurrency)}</strong>
          </div>
        </div>
      `).join("")}
    </div>
...
function renderCompactDetailBlock(item, quoteCurrency) {
  if (item.type === "hotel" && Array.isArray(item.hotelDetails) && item.hotelDetails.length) {
    return renderCompactDetailRows(item.hotelDetails.map(detail => ({
      title: detail.roomType || "酒店明细",
      meta: [
        `${displayValue(detail.roomCount, 0)} 间`,
        `${displayValue(detail.nights, 0)} 晚`,
        window.AppUi.getLabel("currencyLabels", detail.currency),
      ],
      note: detail.notes,
      cost: detail.costSubtotal,
      price: detail.priceSubtotal,
    })), quoteCurrency);
  }

  if (item.type === "vehicle" && Array.isArray(item.vehicleDetails) && item.vehicleDetails.length) {
    return renderCompactDetailRows(item.vehicleDetails.map(detail => ({
      title: `${window.AppUi.getLabel("vehicleDetailTypeLabels", detail.detailType)} / ${detail.vehicleModel || "车辆明细"}`,
      meta: [
        `${displayValue(detail.vehicleCount, 0)} 车`,
        `${displayValue(detail.billingQuantity, 0)} ${window.AppUi.getLabel("vehiclePricingUnitLabels", detail.pricingUnit)}`,
        window.AppUi.getLabel("currencyLabels", detail.currency),
      ],
      note: detail.notes,
      cost: detail.costSubtotal,
  ...
  if (["guide", "interpreter"].includes(item.type) && Array.isArray(item.serviceDetails) && item.serviceDetails.length) {
    return renderCompactDetailRows(item.serviceDetails.map(detail => ({
      title: `${window.AppUi.getLabel("serviceRoleLabels", detail.serviceRole)} / ${window.AppUi.getLabel("serviceLanguageLabels", detail.serviceLanguage)}`,
      meta: [
        window.AppUi.getLabel("serviceDurationLabels", detail.serviceDuration),
        `数量 ${displayValue(detail.quantity, 0)}`,
        window.AppUi.getLabel("currencyLabels", detail.currency),
      ],
      note: detail.notes,
      cost: detail.costSubtotal,
  ...
  if (item.type === "dining" && item.mealDetails) {
    ...
    return renderCompactDetailRows([{
      title: "用餐计价",
      meta: [
        `${displayValue(detail.mealPeople, 0)} 人`,
        `${displayValue(detail.tripDays, 0)} 天`,
        window.AppUi.getLabel("currencyLabels", detail.currency),
      ],
      note: `午餐 ${detail.includeLunch ? "计算" : "不计算"}，晚餐 ${detail.includeDinner ? "计算" : "不计算"}`,
      cost: item.totalCost,
...
function renderQuoteDetailItemCard(item, quote) {
  const itemType = window.AppUi.getLabel("quoteItemTypeLabels", item.type);
  const currency = quote.currency;
  const sourceCurrency = window.AppUi.getLabel("currencyLabels", item.currency);
  const meta = [
    `数量 ${displayValue(item.quantity, 0)} ${displayValue(item.unit, "")}`.trim(),
    `原币种 ${sourceCurrency || displayValue(item.currency)}`,
  ];
  if (item.notes) meta.push(`备注 ${item.notes}`);

  return `
    <article class="quote-detail-item-card">
      <div class="quote-detail-item-header">
        <div>
          <span class="quote-detail-type">${esc(itemType || item.type || "服务")}</span>
          <h3>${esc(item.name || "未命名服务")}</h3>
        </div>
        <div class="quote-detail-amounts">
          <strong>销售 ${formatMoney(item.totalPrice, currency)}</strong>
          <span>成本 ${formatMoney(item.totalCost, currency)}</span>
        </div>
      </div>
      <div class="quote-detail-item-body">
        ${renderDetailMeta(meta)}
      </div>
      ${renderCompactDetailBlock(item, currency)}
    </article>
  `;
}
...
function normalizeQuoteActions() {
  const row = document.getElementById("quote-action-row");
  if (!row) return;

  let primary = document.getElementById("quote-primary-actions");
  let secondary = document.getElementById("quote-secondary-actions");
  let danger = document.getElementById("quote-danger-actions");
  const directActions = Array.from(row.children).filter(action => !action.classList.contains("quote-action-group"));
  if (!primary || !secondary || !danger) {
    row.textContent = "";
    primary = document.createElement("div");
    secondary = document.createElement("div");
    danger = document.createElement("div");
    primary.id = "quote-primary-actions";
    secondary.id = "quote-secondary-actions";
    danger.id = "quote-danger-actions";
    primary.className = "quote-action-group quote-action-primary";
    secondary.className = "quote-action-group quote-action-secondary";
    danger.className = "quote-action-group quote-action-danger";
    row.append(primary, secondary, danger);
  }

  directActions.forEach(action => {
    const href = action.getAttribute("href") || "";
    const text = action.textContent || "";
    action.classList.add("qd-action");
    action.style.removeProperty("margin-left");
    action.style.removeProperty("width");
    if (href.includes("/quote-new") || href.includes("/standard-quotation") || text.includes("分享") || text.includes("审批") || text.includes("批准") || text.includes("重开")) {
      action.classList.add("qd-action-primary");
      primary.appendChild(action);
    } else if (text.includes("删除") || text.includes("拒绝")) {
      action.classList.add("qd-action-danger");
      danger.appendChild(action);
    } else {
      action.classList.add("qd-action-secondary");
      secondary.appendChild(action);
    }
  });
}
...
async function requestCustomerShareToken(quoteId) {
  const payload = await window.AppUtils.fetchJson(
    `/api/customer-standard-quotations/${encodeURIComponent(quoteId)}/share-token`,
    {
      method: "POST",
...
function showCustomerShareDialog(link, expiresAt) {
  const overlay = document.createElement("div");
  overlay.className = "customer-share-overlay";
  overlay.innerHTML = `
...
async function bootstrap() {
  ...
  try {
    ...
    container.classList.add("quote-detail-main");
    ...
    container.innerHTML = `
      <div class="quote-detail-hero">
        ...
        <div class="quote-detail-actions" id="quote-action-row">
      ...
      <div class="panel subpanel section-spacing">
        ...
        <div class="quote-detail-items">
          ${items.length > 0 ? items.map((item) => renderQuoteDetailItemCard(item, quote)).join("") : '<p class="empty">这份报价还没有报价项。</p>'}
        </div>
      </div>
      <div class="panel subpanel section-spacing">
...
document.addEventListener("authReady", function () {
  if (cachedQuote) {
    renderActionButtons(cachedQuote);
    normalizeQuoteActions();
  }
});
